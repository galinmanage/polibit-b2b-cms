...
class TwoActionPopup extends Component {
        ...
        <div className={'popup_wrapper ' + this.state.animation_class} onClick={(e) => e.stopPropagation()}>
          ...
          <div className={'buttons-container'}>
            <button className="button accept-btn teal-button"
                    onClick={() => this.animateOut(this.props.payload.action1)}>{this.props.payload.btnText1}</button>
            <button className="button accept-btn teal-button"
                    onClick={() => this.animateOut(this.props.payload.action2)}>{this.props.payload.btnText2}</button>
          </div>
        </div>
      </div>
    );
...
export default connect(mapStateToProps, mapDispatchToProps, null, { pure: false })(TwoActionPopup);

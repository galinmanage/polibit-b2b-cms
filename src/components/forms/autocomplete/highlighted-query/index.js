...
class AutoComplete extends Component {
  ...
  getPartSpan(text, match, space = false) {
    ...
    if (space) {
      component = <span className="part" key={key}>&nbsp;</span>;
    } else {
      component = <span className={match ? 'match' : 'part'} key={key}>{text}</span>;
    }
    return component;
  }
        ...
        <div className={'auto_wrapper ' + className + ' ' + activeClass + (disabled ? ' disabled' : '')}
          ...
          <ul className="auto_menu">
            {options.map((item, index) => {
              ...
              if (item.id === selected_id) { activeOption = 'active'; }

              if (index === this.state.highlighted_item) { hightlightedOption = ' highlight';}
              ...
              if (query.length > 0) {
                ...
                switch (position) {
                  case 'start':
                    ...
                    this.replaceSpaces(parts, item.text.substring(endIndex, item.text.length));
                    break;
              ...
              return <li className={'auto_option ' + activeOption + hightlightedOption}
                         key={index}
                         id={item.id}
